            ...
            { props.itemsToRender.map((item, ix) => {
                return(
                    <div className=" custom-orange-border car-card p-4" key={ix}>
                        ...
                        <div className="d-flex justify-content-center align-items-center mt-3"><Link className="text-info cursor-pointer" onClick={clickHandler} to={`/car-details/${item.id}`}>Read More</Link></div>
                    </div>
                )
            })}
        </>
